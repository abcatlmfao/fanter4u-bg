...
what is it?
    ...
    has themes, favorites, ratings, accounts, achievements, and a bunch of other stuff i kept adding because it was fun

features so far:

    🎮 game library - loads from a json file, searchable, filterable by favorites

    🎨 theme system - backgrounds (chill, parallax, aurora, void, solid), fonts, accent colors, presets (dark/light/rainbow/neon)

    ⭐ favorites - star your favorite games, filter to show only them, syncs to your account if logged in

    🔑 safety key - double tap any key to instantly redirect to a safe url (for those "uh oh" moments)

    🌐 language - uses your browser's built in translation (no ugly google bar)

    🎲 random game - picks a random game from the library

    📊 game ratings - 1-5 star system, saves globally to jsonbin so everyone sees the same ratings

    👤 accounts - register/login, custom display names, tracks your favorites, played games, and ratings

    🏆 achievements - 60 achievements across 6 categories (newbie, beginner, climbing, difficult, elite, secret), each with point values, secret ones are hidden as "???" until unlocked

    ⏱️ session timer - top left corner, tracks how long you've been actively using the site (pauses when tab is inactive)

    🎯 recently played - saves the last 10 games you clicked on

    🎨 visual settings - accent color, blur intensity, card transparency, border styles, shadows, corner roundness

    🔊 audio settings - hover sounds, background music, click effects, mute all (work in progress but the ui is there)
...
credits for game porting: aukak (bog), corbins98, 
